...
export async function GET(request) {
  try {
    ...
    await addSystemLog('Database JSON Exported', ip, 'Success');
...
export async function POST(request) {
  try {
    ...
    if (action === 'reset') {
      ...
      await addSystemLog('Factory Reset Executed', ip, 'Success');
      return NextResponse.json({ success: true, message: 'System data reset completed successfully.' });
    } else if (action === 'import') {
      if (!data) {
        await addSystemLog('Database Restore Failed (Empty Payload)', ip, 'Failure');
        return NextResponse.json({ success: false, message: 'Import data payload is missing.' }, { status: 400 });
      }
      try {
        await importDataStore(data);
        await addSystemLog('Database Restored from JSON Backup', ip, 'Success');
        return NextResponse.json({ success: true, message: 'System database restored successfully.' });
      } catch (validationError) {
        await addSystemLog(`Database Restore Failed (${validationError.message})`, ip, 'Failure');
